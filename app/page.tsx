...
export default function Home() {
  ...
  async function regenerate(seg: Cut) {
    ...
    try {
      ...
      logUpsert(logId, { title: `Regenerate ${slot}`, detail: `Frames ready · generating ${durationSec}s clip via Codex + Pika (${modelName})`, status: "active", bar: true });
    } catch (error) {
      ...
      <div className="topbar-right">
        ...
        <div className="appearance-control">
          ...
          {showAppearance && <div className="appearance-menu" role="dialog" aria-label="Settings">
            ...
            {REGEN_PROVIDERS.map((model) => <button className={`scheme-option model-option ${model.id === genModel ? "selected" : ""}`} onClick={() => setGenModel(model.id)} key={model.id} aria-pressed={model.id === genModel}>
